...
import { usePointsDisplay } from '../usePointsDisplay';

describe('usePointsDisplay', () => {
  beforeEach(() => vi.useFakeTimers());
  afterEach(() => vi.useRealTimers());

  it('initializes with base value at progress 0', () => {
    const { result } = renderHook(() =>
      usePointsDisplay({ base: 10, steps: 5, progressElapsed: 0 }),
    );
  ...
  it('decreases in discrete steps as progress crosses thresholds', () => {
    const { result, rerender } = renderHook(
      ({ p }) => usePointsDisplay({ base: 10, steps: 5, progressElapsed: p }),
      { initialProps: { p: 0 } },
    );
  ...
  it('guards invalid steps: floors and min 1', () => {
    ...
    const { result, rerender } = renderHook(
      ({ s, p }) =>
        usePointsDisplay({ base: 10, steps: s, progressElapsed: p }),
