...
import { useStepPoints } from '../useStepPoints';

describe('useStepPoints', () => {
  beforeEach(() => vi.useFakeTimers());
  afterEach(() => vi.useRealTimers());

  it('initializes with base value at progress 0', () => {
    const { result } = renderHook(() =>
      useStepPoints({ base: 10, steps: 5, progressElapsed: 0 }),
    );
  ...
  it('decreases in discrete steps as progress crosses thresholds', () => {
    const { result, rerender } = renderHook(
      ({ p }) =>
        useStepPoints({ base: 10, steps: 5, progressElapsed: p, fadeMs: 200 }),
      { initialProps: { p: 0 } },
    );
  ...
  it('clamps progressElapsed to [0,1]', () => {
    const { result, rerender } = renderHook(
      ({ p }) => useStepPoints({ base: 10, steps: 5, progressElapsed: p }),
      { initialProps: { p: -5 } },
    );
  ...
  it('guards invalid steps: floors and min 1', () => {
    ...
    const { result, rerender } = renderHook(
      ({ s, p }) => useStepPoints({ base: 10, steps: s, progressElapsed: p }),
